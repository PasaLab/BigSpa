...
public class Printer {
	...
	final private static char fileSeparator = System.getProperty(
	...
	public void printTo(SootClass cl, PrintWriter out) {
		...
		{
			StringTokenizer st = new StringTokenizer(Modifier.toString(cl
					.getModifiers()));
	...
	/**
	 * Prints out the method corresponding to b Body, (declaration and body), in
	 * the textual format corresponding to the IR used to encode b body.
	 * 
	 * @param out
	...
	/**
	 * Prints the given <code>JimpleBody</code> to the specified
	 * <code>PrintWriter</code>.
	 */
	private void printStatementsInBody(Body body, java.io.PrintWriter out,
			LabeledUnitPrinter up, UnitGraph unitGraph) {
		...
		while (unitIt.hasNext()) {
			...
			{
				// Put an empty line if the previous node was a branch node, the
				// current node is a join node
				// or the previous statement does not have body statement as a
				// successor, or if
				...
				if (currentStmt != units.getFirst()) {
					if (unitGraph.getSuccsOf(previousStmt).size() != 1
						...
						// Or if the previous node does not have body statement
						// as a successor.
			...
			if (Options.v().print_tags_in_output()) {
				 ...
				 * while (udIt.hasNext()) { ValueBox temp =
				 * (ValueBox)udIt.next(); Iterator vbtags =
				 * temp.getTags().iterator(); while (vbtags.hasNext()) { Tag t =
				 * (Tag) vbtags.next(); up.noIndent();
				 * up.literal("VB Tag: "+t.toString()); up.newline(); } }
				 */
			}
		}
		...
		// Print out exceptions
		{
			...
			while (trapIt.hasNext()) {
				...
				out.println("        catch "
						...
						+ " from " + up.labels().get(trap.getBeginUnit())
						+ " to " + up.labels().get(trap.getEndUnit())
						+ " with " + up.labels().get(trap.getHandlerUnit())
						+ ";");
	...
	private void printLocalsInBody(Body body, UnitPrinter up) {
		...
		{
			Map<Type, List> typeToLocals = new DeterministicHashMap(
