...
object Graspan_pureRDD extends Para{

  def processGrammar(grammar_origin:List[Array[String]],input_grammar:String)
  :(Map[String,EdgeLabel],Int,List[EdgeLabel],Map[EdgeLabel,EdgeLabel],List[((EdgeLabel,EdgeLabel),EdgeLabel)])={
    ...
    (symbol_Map,symbol_num,loop,directadd,grammar)
  }
  def processGraph(graph_origin:RDD[(Int,Int,String)],input_grammar:String,symbol_Map:Map[String,EdgeLabel],
                   loop:List[EdgeLabel],
                   directadd:Map[EdgeLabel,EdgeLabel],par:Int):(RDD[(VertexId,VertexId,EdgeLabel)],Int)={
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (symbol_Map,symbol_num,loop,directadd,grammar)=processGrammar(grammar_origin,input_grammar)
    ...
    val(graph,nodes_totalnum)=processGraph(graph_origin,input_grammar,symbol_Map,loop,
      directadd,par)
    ...
    while(continue){
      ...
      val newedges_dup_str=(oldedges ++ newedges).flatMap(s=>List((s._1,((s._1,s._2),s._3,s._4)),(s._2,
        ...
        .repartition(par)
        .mapPartitionsWithIndex((index,s)=>computeInPartition_completely(step,index,s,grammar,directadd,
      ...
      val alledges=(oldedges.map(s=>(s._1,s._2,s._3)) ++ newedges.map(s=>(s._1,s._2,s._3)))
      oldedges=(oldedges ++ newedges.map(s=>(s._1,s._2,s._3,false))).cache()
      newedges=newedges_removedup.subtract(alledges).map(s=>(s._1,s._2,s._3,true))
      println("newedges count to: "+newedges.count())
