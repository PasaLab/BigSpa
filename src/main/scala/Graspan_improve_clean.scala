...
object Graspan_improve_clean extends Para{
  ...
  def main(args: Array[String]): Unit = {
    ...
    while (continue) {
      ...
      val par_time_JOIN=new_edges_str.map(s=>s._2._2).map(s=>s.split("REPARJOIN")(1).trim.toDouble.toInt).collect().sorted
