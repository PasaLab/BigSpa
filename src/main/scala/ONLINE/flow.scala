...
object flow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var addturn=0
    for(add<-f){
//      val part=add.toString.split("\\")
      println("\n************During add turn " + addturn + s"\t$add\t************")
      flowadd=0

      val t0_delta=System.nanoTime()
      var add_edges: Array[(Int, Int, Int)] = Source.fromFile(add).getLines().toArray.map(line => {
        val s = line.split("\\s+")
...
////          if(addturn==617){
////            println("add_edges:")
////            add_edges.foreach(e=>println(e._1+"\t"+e._2+"\t"+e._3))
////          }
...
//    println("Query for Compute: "+Query.length)
    //    +"-"+Query.mkString(" "))
    val Query_Answer:Map[Long,java.util.Map[Integer,java.lang.Long]]=redis_OP.Query_PB_Array(Query)//MuitiThread
//    val t1_3=System.nanoTime()
//    println(" \t3: Query Redis for Compute uses \t"+(t1_3-t0_3)/1e9)
