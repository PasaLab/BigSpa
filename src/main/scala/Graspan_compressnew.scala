...
object Graspan_compressnew extends Para{
    ...
    while (continue) {
      ...
      val new_edges_str = oldedges
        .mapPartitionsWithIndex((index, s) =>
          Graspan_OP.computeInPartition_compressnew(step,
            index, s,
            symbol_num,grammar,
            nodes_num_bitsize,
            symbol_num_bitsize, directadd)).setName("newedge-before-distinct-" + step)
